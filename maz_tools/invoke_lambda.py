...
def invoke_lambda(image_path, output_dir, function_name='te-advent-tm-v2', region='eu-central-1'):
    """
    ...
    output_image = base64.b64decode(body['result']['image'])
    output_path = os.path.join(output_dir, f"{os.path.basename(image_path)}_detected_lambda_{body['result']['detection_time']:.3f}_{memory_used or '0'}.png")
    
    with open(output_path, 'wb') as f:
        f.write(output_image)

    bboxes = body['result'].get('bboxes', [])
    if 0 < len(bboxes):
    ...
    print(f"{image_path},{body['result']['detection_time']:.3f},{body['result']['handling_time']:.3f},{memory_used or '0'},{body['result']['image_size']['width']}x{body['result']['image_size']['height']}")
...
def main():
    ...
    function_name = sys.argv[3] if len(sys.argv) > 3 else 'te-advent-tm-v2'
    region = sys.argv[4] if len(sys.argv) > 4 else 'eu-central-1'
    ...
    try:
        invoke_lambda(image_path, output_dir, function_name, region)
    except Exception as e:
        print(f"\nError: {str(e)}")
        import traceback
        traceback.print_exc()
