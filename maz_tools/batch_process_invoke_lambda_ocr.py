...
Usage:
    python batch_process_invoke_lambda_ocr.py <input_folder> <chunk_size>

Example:
    python batch_process_invoke_lambda_ocr.py ./qa_input 5
"""
...
def process_file(image_path):
    ...
    try:
        ...
        cmd = [sys.executable, 'invoke_lambda_OCR.py', str(image_path)]
...
def process_chunk(chunk, chunk_idx, total_chunks):
    ...
    with ThreadPoolExecutor(max_workers=len(chunk)) as executor:
        # Submit all tasks
        futures = {
            executor.submit(process_file, img_path): img_path
...
def main():
    """Main function"""
    if len(sys.argv) != 3:
        print("Usage: python batch_process_invoke_lambda_ocr.py <input_folder> <chunk_size>")
        print("\nExample:")
        print("  python batch_process_invoke_lambda_ts.py ./qa_input 5")
        print("\nArguments:")
    ...
    for idx, chunk in enumerate(chunks, 1):
        chunk_results = process_chunk(chunk, idx, total_chunks)
