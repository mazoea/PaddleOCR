...
import numpy as np
import cv2
from bbox_from_ocr_response import get_word_bboxes
    ...
    for scale in [0.2, 0.4, 0.6, 1.]:
        ...
        for angle in [0, 90, 180, 270]:
            if angle == 0:
                ...
                rotated_img = cv2.warpAffine(crop_img, rot_matrix, (crop_img.shape[1], crop_img.shape[0]), flags=cv2.INTER_LINEAR)

            # detect text in the cropped image
            confs = []
            ts_res = text_detector.predict(rotated_img)
            for i, bbox in enumerate(ts_res[0]["dt_polys"], 1):
                ...
                word_img = rotated_img[y:y+h_box, x:x+w_box]
                ...
                for res in ocr_res:
                    ...
                    confs.append(res.get('rec_score', 0.0))
        ...
        avg_confs = [avg / max_word_counts for avg in avg_confs]
